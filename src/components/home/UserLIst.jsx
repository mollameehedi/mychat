    ...
    onValue(fRequestRef, (snapshot) => {
      ...
      snapshot.forEach((item) => {
        if (item.val().receiver_id == data.uid) {
          xyz.push(item.val().sender_id + item.val().receiver_id)
        }
              ...
              <div className='userinfobox'>
                ...
                {fRequest.length > 0 && fRequest.includes(user.id + data.uid) || fRequest.includes(data.uid + user.id) ?
                  ...
                  friendList.includes(user.id + data.uid) || friendList.includes(data.uid + user.id) ?
                    ...
                    :
                    requestCheck && requestCheck.includes(user.id + data.uid)
                      ?
                      <>
                        <button onClick={() => handleCencelReqeust(user)} className='addbutton text-danger' >Confrim Request</button>
                      </>
                      :
